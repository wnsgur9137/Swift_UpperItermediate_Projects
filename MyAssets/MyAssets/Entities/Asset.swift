...
class AssetData: Identifiable, ObservableObject, Decodable {
    ...
    let title: String
    let amount: String
    let creditCardAmonuts: [CreditCardAmounts]
    
    init(id: Int, title: String, amount: String, creditCardAmonuts: [CreditCardAmounts]? = nil) {
        self.id = id
        self.title = title
        self.amount = amount
        self.creditCardAmonuts = creditCardAmonuts!
    }
}
